 ...
 */
public class LoadMoreGrideView extends GridViewWithHeaderAndFooter implements AbsListView.OnScrollListener {
    ...
    public LoadMoreGrideView(Context context, AttributeSet attrs) {
        super(context, attrs);
        //动态载入底部布局
    ...
    public void setRefreshAndLoadMoreView(RefreshAndLoadMore_GrideView mRefreshAndLoadMoreGrideView) {
        this.mRefreshAndLoadMoreGrideView = mRefreshAndLoadMoreGrideView;
    }
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
            if (view.getLastVisiblePosition() == view.getCount() - 1 && (mRefreshAndLoadMoreGrideView != null &&
                    !mRefreshAndLoadMoreGrideView.isRefreshing()) && !isLoading && mOnLoadMoreListener != null && isHaveMoreData) {
                isLoading = true;
