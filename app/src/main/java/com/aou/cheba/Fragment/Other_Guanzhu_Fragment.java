...
public class Other_Guanzhu_Fragment extends Fragment {
    ...
    private List<MyCode_fensi.ObjBean> guanzhu_list = new ArrayList<>();
    private int page = 1;
    ...
    class MyAdapter extends BaseAdapter {
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            if ((guanzhu_list.get(position).getGoId() + "").equals(SPUtils.getString(getActivity(), "uid"))) {
                tv_guanzhu.setVisibility(View.INVISIBLE);
            } else {
                if (guanzhu_list.get(position).isCollected()) {
                        ...
                        if (0 == mycode.getCode()) {
                            ...
                            for (int i = 0; i < new ZiXun_Fragment().list_data.size(); i++) {
                                if (new ZiXun_Fragment().list_data.get(i).getUid() == uid) {
                                    new ZiXun_Fragment().list_data.get(i).setFollowed(true);
                                }
                            }
                            ...
                            guanzhu_list.get(position).setCollected(true);
                            myAdapter.notifyDataSetChanged();
